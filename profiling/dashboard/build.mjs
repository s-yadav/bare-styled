// Builds the dashboard harness into a single browser bundle.
//   1. Babel-compile dashboard.jsx two ways (styled-components / just-styled).
...
// Run:  node profiling/dashboard/build.mjs   (from the repo root)
...
const src = readFileSync(R('dashboard.jsx'), 'utf8')

const sc = transformSync(src, {
  filename: R('dashboard.jsx'), babelrc: false, configFile: false,
  presets: [[presetReact, { runtime: 'automatic' }]],
}).code
writeFileSync(R('.build/sc-dashboard.js'), sc)
...
writeFileSync(R('.build/js-dashboard.js'), js)
...
console.log('\nBuilt profiling/dashboard/dist/harness.js — open profiling/dashboard/harness.html')
